...
# send max and min values of each field instead of the whole dataset
def get_clean_data():
...
def add_sidebar():
    ...
    for label, field in slider_labels:
        input_dict[field] = st.sidebar.slider(
            label, 
            float(data[field].min()), 
            float(data[field].max()), 
            float(data[field].mean())
        )
    return input_dict
